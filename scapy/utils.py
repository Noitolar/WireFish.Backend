...
def hexdump(x, dump=False):
    ...
    :returns: a String only when tmp=True
    """
    s = ""
    x = bytes_encode(x)
    x_len = len(x)
...
def chexdump(x, dump=False):
    ...
    :param x: a Packet
    :param dump: print the view if False
    :returns: a String only if tmp=True
    """
    x = bytes_encode(x)
...
tmp:    when set to True, returns a string instead of displaying it.
...
wait: If True (default), waits for the process to terminate before returning
      ...
      tmp, getproc or getfd is True, these have the same effect as
      ``wait=False``.
