...
def scan(sock, scan_range=range(0x800), noise_ids=None, sniff_time=0.1,
    ...
    for value in scan_range:
        sock.sniff_realtime(prn=lambda pkt: get_isotp_fc(value, return_values,
                                                         noise_ids, False, pkt,
                                                         verbose),
                            timeout=sniff_time,
...
def scan_extended(sock, scan_range=range(0x800), scan_block_size=100,
    ...
    for value in scan_range:
        ...
        for extended_id in range(0, 256, scan_block_size):
            sock.sniff_realtime(prn=lambda p: get_isotp_fc(extended_id, id_list,
                                                           noise_ids, True, p,
                                                           verbose),
        ...
        for extended_id in id_list:
            for ext_id in range(extended_id, min(extended_id +
                ...
                sock.sniff_realtime(prn=lambda pkt: get_isotp_fc(full_id,
                                                                 return_values,
                                                                 noise_ids, True,
                                                                 pkt, verbose),
                                    timeout=sniff_time,
...
def ISOTPScan(sock, scan_range=range(0x7ff + 1), extended_addressing=False,
    ...
    background_pkts = sock.sniff_realtime(timeout=noise_listen_time,
